...
# Global setting for tool visibility (default to True for dev, set to False in prod)
SHOW_TOOL_CALLS = os.getenv("SHOW_TOOL_CALLS", "true").lower() in (
    "true",
    "1",
    "yes",
    "t",
...
def display_chat_message(msg):
    ...
    if isinstance(msg, HumanMessage):
        ...
        with st.chat_message("assistant"):
            ...
            # Show tool calls nicely if SHOW_TOOL_CALLS is True
            if SHOW_TOOL_CALLS and hasattr(msg, "tool_calls") and msg.tool_calls:
                for tc in msg.tool_calls:
                    with st.expander(f"🛠️ Calling Tool: `{tc['name']}`"):
                        st.json(tc["args"])

    elif isinstance(msg, ToolMessage):
        # Only show tool results if SHOW_TOOL_CALLS is True
        if SHOW_TOOL_CALLS:
